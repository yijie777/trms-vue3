          ...
          <a-row>
            ...
            <a-col :span="24">
              <a-form-item label="选项">
                <a-button @click="questionItemAdd()" type="primary">添加选项</a-button>
              </a-form-item>
              <a-form-item :label="item.prefix" v-for="(item,index) in formData.itemsObject"
                ...
                <a-row :gutter="9">
                  ...
                  <a-col :span="3">
                    <a-button @click="questionItemRemove(index)" type="primary" danger>删除选项
                    </a-button>
                  </a-col>
                </a-row>
              </a-form-item>
            ...
            <a-col :span="24">
              <a-form-item label="标答" v-bind="validateInfos.correct">
                <a-checkbox-group v-model:value="formData.correct"  name="checkboxgroup"  >
                  <a-checkbox :value="item.prefix" v-for="item in formData.itemsObject" >{{item.prefix}}</a-checkbox>
                </a-checkbox-group>
              </a-form-item>
            </a-col>
            <a-col :span="24">
...
const validatorRules = {
  ...
  correct: [
    {required: true, message: '标答不能为空', trigger: 'blur'},
  ],
};
...
function init() {
  ...
  if (id) {
    ...
    defHttp.get({url: '/trms/trmsExamQuestion/queryById?id=' + id, params: {}}).then((res) => {
      ...
      res.correct=JSON.parse(res.correct)
...
function questionItemRemove(index) {
  formData.itemsObject.splice(index, 1)
}

function questionItemAdd() {
  let items = formData.itemsObject
  let newLastPrefix
  if (items.length > 0) {
...
async function submitForm() {
  ...
  formData.items = JSON.stringify(formData.itemsObject)
  formData.correct = JSON.stringify(formData.correct)
